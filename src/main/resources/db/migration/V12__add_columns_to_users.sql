...
--
-- 회원가입은 login_id만으로 이루어지고, 프로필 컬럼(protection_end_date 이하)은
-- 이후 진단(intake) 제출 시점에 채워지므로 NOT NULL로 두지 않는다.
--
...
ALTER TABLE users
    ...
    ADD COLUMN name VARCHAR(20) NULL COMMENT '이름',
    ADD COLUMN birth_date CHAR(8) NULL COMMENT '생년월일(yyyyMMdd)',
    ADD COLUMN phone_number CHAR(11) NULL COMMENT '휴대폰 번호(하이픈 제외, 010XXXXXXXX)',
