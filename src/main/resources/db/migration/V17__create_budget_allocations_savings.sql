-- V17__create_budget_allocations_savings.sql
-- 예산 배분/적립 도메인 테이블 생성 (씨앗 1차.sql ERD 기준)
-- MySQL 8.x / Flyway
--
-- ERD의 budget_allocations.latest_diagnosis_id는 VARCHAR(255)로 되어 있었으나,
-- diagnosis.diagnosis_id가 BIGINT라 타입 불일치였다.
-- 이미 마이그레이션된 recommendations.diagnosis_id(BIGINT)와 동일하게 맞춘다.
--
-- policy_matches는 삭제하기로 결정하여 이 마이그레이션에서 제외한다.
